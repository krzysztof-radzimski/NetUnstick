...
struct ReportPreviewView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("report_preview").font(.title.bold()).accessibilityIdentifier("report.preview")
            ...
            HStack { Spacer(); Button("cancel") { dismiss() }.accessibilityIdentifier("report.cancel"); Button("save_report") {
                save()
            }.buttonStyle(.borderedProminent).accessibilityIdentifier("report.save") }
        }.padding(28).frame(width: 650, height: 520)
    }
}
